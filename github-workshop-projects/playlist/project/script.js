...
tracks.forEach(track => {
  ...
  // HINT: Use button.addEventListener('click', function() { ... })
   button.addEventListener('click', function() {
    ...
    if (action === 'play') {
      playTrack(track, audio, button);
    } else {
      pauseTrack(track, audio, button);
    }
  });
...
function playTrack(track, audio, button) {
  ...
  // Then call pauseTrack() for that one
   if (currentlyPlaying && currentlyPlaying !== audio) {
    const prevTrack = document.querySelector('.track.playing');
    const prevButton = prevTrack?.querySelector('.control-btn');
    const prevAudio = currentlyPlaying;
    pauseTrack(prevTrack, prevAudio, prevButton);
  }
